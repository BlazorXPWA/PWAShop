namespace ProPWAShop.Services.Products
{
    using System;
    using System.Linq;
    ...
    public class ProductsService : BaseService<Product>, IProductsService
    {
        ...
        public ProductsService(ProPWAShopDbContext db, IMapper mapper)
            : base(db, mapper)
        {
        }
            ...
            int id, ProductsRequestModel model)
        {
            ...
            product.InPack = model.InPack;
            product.UnitsType = model.UnitsType;
            product.DaysFrom = model.DaysFrom;
            product.DaysTo = model.DaysTo;
