namespace ProPWAShop.Web.Client.Shared.Products
{
    ...
    public partial class AddToCartForm
    {
        ...
        private async Task OnDeleteAsync()
        {
            ...
            if (confirmed)
            {
                ...
                if (result.Succeeded)
                {
                    this.ToastService.ShowSuccess($"Товар был исключен.", $"{this.ProductName}");
        ...
        private async Task AddToWishlist()
        {
            ...
            if (result.Succeeded)
            {
                this.ToastService.ShowSuccess($"Товар был добавлен в список желаний", $"{this.ProductName}");
            }
            else
            {
                this.Errors = result.Errors;
